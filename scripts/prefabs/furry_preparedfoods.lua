...
local function MakePreparedFood(data)
    ...
    local function fn()
        ...
        if spicename ~= nil then
            ...
            food_symbol_build = data.overridebuild or "furry_foods"
        else
            inst.AnimState:SetBuild(data.overridebuild or "furry_foods")
            inst.AnimState:SetBank("furry_foods")
        end

        inst.AnimState:PlayAnimation("idle")
        inst.AnimState:OverrideSymbol("swap_food", data.overridebuild or "furry_foods", data.basename or data.name)
        ...
        inst.food_symbol_build = food_symbol_build or data.overridebuild or "furry_foods"
...
for k, v in pairs(require("spicedfoods")) do
    if v.basename and foods[v.basename] then
        table.insert(prefs, MakePreparedFood(v))
        v.official = false
    end
end
